...
use engine::src::globals::{white_moves___, black_moves___, pv___, pv_depth___, board___, piece_count___};
...
pub unsafe fn ponder_move<
    ...
    while force_return == 0 && i < expect_count {
        ...
        if i == 0 as i32 && force_return == 0 {
            ...
            best_pv_depth = pv_depth___[0];
            j = 0;
            while j < pv_depth___[0] {
                best_pv[j as usize] =
                    pv___[0][j as usize];
                j += 1
            }
...
pub unsafe fn get_pv(destin: &mut [i32]) -> i32 {
    ...
    return if prefix_move == 0 {
        ...
        while i < pv_depth___[0] {
            destin[i as usize] = pv___[0][i as usize];
            i += 1
        }
        pv_depth___[0]
    } else {
        ...
        i = 0;
        while i < pv_depth___[0] {
            destin[(i + 1 as i32) as usize] = pv___[0][i as usize];
            i += 1
        }
        pv_depth___[0] + 1
    };
                ...
                if shallow_depth == 1 as i32 {
                    ...
                    if shallow_info.type_0 as u32 ==
                        ...
                        if shallow_info.type_0 as u32 ==
                            ...
                            /* Game over */
                            disc_diff =
                                disc_count(side_to_move, &board___) -
                                    disc_count(0 as i32 +
                                                   2 as i32 -
    ...
    if evaluated_list[0].eval.type_0 as u32
        ...
        if evaluated_list[0].eval.type_0 as
            ...
            if disc_diff > 0 as i32 {
                ...
                corrected_diff =
                    2 as i32 * disc_count(side_to_move, &board___) -
                        64 as i32;
                res = LOST_POSITION
            }
            evaluated_list[0].eval =
...
fn send_move_type_0_status(interrupted_depth: i32, info: &EvaluationType, counter_value: f64, elapsed_time: f64) {
    unsafe {
        ...
        send_status_pv(&mut pv___[0], interrupted_depth, pv_depth___[0]);

...
pub fn constant_and_parity_feature(side_to_move: i32, disks_played: i32,
    ...
    let x: &[(&[i16], &[usize])] = &[
        ...
        (&*set.afile2x(), &[77, 72, 88, 87, 86, 85, 84, 83, 82, 81]),
        (&*set.bfile(), &[82, 72, 62, 52, 42, 32, 22, 12]),
        (&*set.bfile(), &[87, 77, 67, 57, 47, 37, 27, 17]),
        (&*set.bfile(), &[28, 27, 26, 25, 24, 23, 22, 21]),
        (&*set.bfile(), &[78, 77, 76, 75, 74, 73, 72, 71]),
        (&*set.cfile(), &[83, 73, 63, 53, 43, 33, 23, 13]),
        (&*set.cfile(), &[86, 76, 66, 56, 46, 36, 26, 16]),
        (&*set.cfile(), &[38, 37, 36, 35, 34, 33, 32, 31]),
        (&*set.cfile(), &[68, 67, 66, 65, 64, 63, 62, 61]),
        (&*set.dfile(), &[84, 74, 64, 54, 44, 34, 24, 14]),
        (&*set.dfile(), &[85, 75, 65, 55, 45, 35, 25, 15]),
        (&*set.dfile(), &[48, 47, 46, 45, 44, 43, 42, 41]),
        (&*set.dfile(), &[58, 57, 56, 55, 54, 53, 52, 51]),
        (&*set.diag8(), &[88, 77, 66, 55, 44, 33, 22, 11]),
        (&*set.diag8(), &[81, 72, 63, 54, 45, 36, 27, 18]),
        (&*set.diag7(), &[78, 67, 56, 45, 34, 23, 12]),
        (&*set.diag7(), &[87, 76, 65, 54, 43, 32, 21]),
        (&*set.diag7(), &[71, 62, 53, 44, 35, 26, 17]),
        (&*set.diag7(), &[82, 73, 64, 55, 46, 37, 28]),
        (&*set.diag6(), &[68, 57, 46, 35, 24, 13]),
        (&*set.diag6(), &[86, 75, 64, 53, 42, 31]),
        (&*set.diag6(), &[61, 52, 43, 34, 25, 16]),
        (&*set.diag6(), &[83, 74, 65, 56, 47, 38]),
        (&*set.diag5(), &[58, 47, 36, 25, 14]),
        (&*set.diag5(), &[85, 74, 63, 52, 41]),
        (&*set.diag5(), &[51, 42, 33, 24, 15]),
        (&*set.diag5(), &[84, 75, 66, 57, 48]),
        (&*set.diag4(), &[48, 37, 26, 15]),
        (&*set.diag4(), &[84, 73, 62, 51]),
        (&*set.diag4(), &[41, 32, 23, 14]),
        (&*set.diag4(), &[85, 76, 67, 58]),
        (&*set.corner33(), &[33, 32, 31, 23, 22, 21, 13, 12, 11]),
        (&*set.corner33(), &[63, 62, 61, 73, 72, 71, 83, 82, 81]),
        (&*set.corner33(), &[36, 37, 38, 26, 27, 28, 16, 17, 18]),
        (&*set.corner33(), &[66, 67, 68, 76, 77, 78, 86, 87, 88]),
        (&*set.corner52(), &[25, 24, 23, 22, 21, 15, 14, 13, 12, 11]),
        (&*set.corner52(), &[75, 74, 73, 72, 71, 85, 84, 83, 82, 81]),
        (&*set.corner52(), &[24, 25, 26, 27, 28, 14, 15, 16, 17, 18]),
        (&*set.corner52(), &[74, 75, 76, 77, 78, 84, 85, 86, 87, 88]),
        (&*set.corner52(), &[52, 42, 32, 22, 12, 51, 41, 31, 21, 11]),
        (&*set.corner52(), &[57, 47, 37, 27, 17, 58, 48, 38, 28, 18]),
        (&*set.corner52(), &[42, 52, 62, 72, 82, 41, 51, 61, 71, 81]),
        (&*set.corner52(), &[47, 57, 67, 77, 87, 48, 58, 68, 78, 88])
    ];
    let mut x = x.into_iter();
    /* The pattern features. */
    return if side_to_move == 0 as i32 {
        while let Some((set_item, indexes)) = x.next() {
            score = update_score(board, score, set_item, indexes);
        }
        score as i32
    } else {
        while let Some((set_item, indexes)) = x.next() {
            score = update_score_back(board, score, set_item, indexes);
        }
        score as i32
    }
}

...
unsafe fn solve_parity_hash_high(end: &mut End, my_bits: BitBoard,
    ...
    DoFlips_hash(sq, color, &mut board_state.board, &mut hash_state.hash_flip1, &mut hash_state.hash_flip2);
...
unsafe fn end_tree_search<FE: FrontEnd>(end: &mut End,level: i32,
    ...
    *selective_cutoff = 0;
    ...
    my_discs = board_state.piece_count[side_to_move as usize][disks_played as usize];
    opp_discs =
    ...
    loop  {
        ...
        if update_pv != 0 {
            ...
            i = level + 1 as i32;
            while i < board_state.pv_depth[(level + 1 as i32) as usize] {
                board_state.pv[level as usize][i as usize] =
                    board_state.pv[(level + 1 as i32) as usize][i as usize];
                i += 1
            }
        }
        if best >= beta {
...
pub unsafe fn end_game<FE: FrontEnd>(side_to_move: i32,
    ...
    board_state.piece_count[0][disks_played as usize] = disc_count(0, &board_state.board);
    board_state.piece_count[2][disks_played as usize] = disc_count(2, &board_state.board);
    if empties > 32 {
    ...
    if is_panic_abort() == 0 && force_return == 0 {
        ...
        if is_panic_abort() == 0 && force_return == 0 {
            ...
            if search_state.root_eval < 0 as i32 {
                res_0 = LOST_POSITION
            } else if search_state.root_eval == 0 as i32 {

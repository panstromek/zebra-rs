...
use crate::src::globals::{board___, piece_count___};
...
pub unsafe fn make_move(side_to_move: i32,
    ...
    let mut diff2: u32 = 0;
    if update_hash != 0 {
        flipped = DoFlips_hash(move_0, side_to_move, &mut board___, &mut hash_state.hash_flip1, &mut hash_state.hash_flip2);
        ...
        flipped = DoFlips_no_hash(move_0, side_to_move, &mut board___);
        if flipped == 0 as i32 { return 0 as i32 }
        hash_state.hash_stored1[disks_played as usize] = hash_state.hash1;
        hash_state.hash_stored2[disks_played as usize] = hash_state.hash2
    }
    flip_count[disks_played as usize] = flipped;
    board___[move_0 as usize] = side_to_move;
    if side_to_move == 0 as i32 {
        piece_count___[0][(disks_played + 1 as i32) as usize] =
            piece_count___[0][disks_played as usize] +
                flipped + 1 as i32;
        piece_count___[2][(disks_played + 1 as i32) as usize] =
...
pub unsafe fn unmake_move(side_to_move: i32,
    ...
    if UndoFlips__flip_count & 1 as i32 != 0 {
        ...
        board___[global_flip_stack[flip_stack]] = UndoFlips__oppcol
    }
...
pub unsafe fn generate_specific(curr_move: i32, side_to_move: i32) -> i32 {
    ...
    return AnyFlips_compact(&board___, inc, curr_move, side_to_move,
                            0 as i32 + 2 as i32 - side_to_move);
}
/*
   GENERATE_MOVE
...
pub unsafe fn generate_move(side_to_move: i32)
    ...
    while move_index < 60 as i32 {
        ...
        if board___[move_0 as usize] == 1 as i32 &&
...
pub unsafe fn valid_move(move_0: i32,
    ...
    while i < 8 as i32 {
        if dir_mask[move_0 as usize] & (1 as i32) << i != 0 {
            ...
            count = 0;
            while board___[pos as usize] ==
                0 as i32 + 2 as i32 - side_to_move {
                pos += move_offset[i as usize];
                count += 1
            }
            if board___[pos as usize] == side_to_move {

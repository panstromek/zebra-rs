...
pub unsafe fn eval_adjustment(disc_adjust: f64,
    ...
    while i < stage_count - 1 as i32 {
        ...
        j = 0;
        let stage_set = set[stage[i as usize] as usize].data.as_mut().unwrap();
        let sixty_set = set[60].data.as_mut().unwrap();
...
pub unsafe fn pattern_evaluation<FE: FrontEnd>(side_to_move: i32) -> i32 {
    ...
    constant_and_parity_feature(side_to_move, disks_played, &mut globals::board_state.board, &mut set[eval_phase as usize])
}

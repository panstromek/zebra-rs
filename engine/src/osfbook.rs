...
pub unsafe fn get_hash(val0: &mut i32, val1: &mut i32, orientation: &mut i32) {
    get_hash_safe(val0, val1, orientation, &mut g_book, &board_state.board___)
}
...
pub unsafe fn find_opening_name() -> *const i8 {
    ...
    get_hash_safe(&mut val1, &mut val2, &mut orientation, &mut g_book, &board_state.board___);
    let mut i = 0;
...
pub unsafe fn check_forced_opening<FE: FrontEnd>(side_to_move: i32,
    ...
    while symm_index < 8 as i32 {
        ...
        while i <= 8 as i32 && same_position != 0 {
            ...
            while j <= 8 as i32 {
                pos = 10 as i32 * i + j;
                if board_state.board___[pos as usize] !=
...
pub unsafe fn fill_move_alternatives<FE: FrontEnd>(side_to_move: i32,
    ...
    let mut book = &mut g_book;
    let board1 = &board_state.board___;
    ...
    if alternative_move > 0 as i32 {
        ...
        alternative_score =
            adjust_score((*book.node.offset(index as isize)).alternative_score as
                             i32, side_to_move, &mut book, disks_played)
...
pub unsafe fn get_book_move<FE: FrontEnd>(mut side_to_move: i32,
    ...
    *eval_info =
                ...
                alternative_score =
                    adjust_score((*book.node.offset(*book.book_hash_table.offset(slot as
                        ...
                        isize)).alternative_score
                                     as i32, side_to_move, book, disks_played)
            } else { alternative_score = -(12345678 as i32) }
    ...
    board_state.pv_depth___[0] = level;
    i = 0;
    while i < level {
        board_state.pv___[0][i as usize] = temp_move[i as usize];
        i += 1
    }
    loop  {
        level -= 1;

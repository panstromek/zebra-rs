use crate::{
    src::{
        ...
        globals::{piece_count___, board___, pv___, pv_depth___},
...
pub unsafe fn tree_search<FE: FrontEnd>(level: i32,
    ...
    if remains < 3 as i32 {
        ...
        pv___[level as usize][level as usize] = midgame_state.best_mid_move;
        return curr_val
    }
    search_state.nodes.lo = search_state.nodes.lo.wrapping_add(1);
    ...
    /* Try the hash table move(s) first if feasible */
    {
        ...
        if update_pv != 0 {
            ...
            j = level + 1 as i32;
            while j < pv_depth___[(level + 1 as i32) as usize] {
                pv___[level as usize][j as usize] =
                    pv___[(level + 1 as i32) as usize][j as usize];
                j += 1
            }
        }
        if best >= beta {
...
unsafe fn fast_tree_search<FE: FrontEnd>(level: i32,
    ...
    if search_state.nodes.lo & 4095 as i32 as u32 == 0 as i32 as u32 {
        reorder_move_list(&board___, &mut search_state.sorted_move_order[disks_played as usize]);
    }
    /* Search */
    first = 1;
    best_move = -(1 as i32);
